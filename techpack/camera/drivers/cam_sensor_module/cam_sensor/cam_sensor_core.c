...
#include <linux/hardware_info.h>//added by wangxiangqin.wt on 20211022, add camera hardware info

#define __ENABLE_CHECK_SUPPLIER_ID__ //bug 695521, litao.wt, ADD, 2021/10/7, distinguish ov13b10 supplier id.
#ifdef __ENABLE_CHECK_SUPPLIER_ID__
#define OV13B10_SENSOR_ID (0x0d42)
#define OFILM_SUPPLIER_ID (0x07)
#define TXD_SUPPLIER_ID (0x0B)
#define OV13B10_TXD_I2C_ADDR (0x20)
#define OV13B10_OFILM_I2C_ADDR (0x6C)
#endif
	...
	struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	s_ctrl->pipeline_delay =
		probe_info->reserved;
        s_ctrl->sensordata->slave_info.camera_id = probe_info->camera_id;
...
int cam_sensor_match_id(struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	if (cam_sensor_id_by_mask(s_ctrl, chipid) != slave_info->sensor_id) {
	    ...
	    #ifdef __ENABLE_CHECK_SUPPLIER_ID__
        if ((slave_info->sensor_id == OV13B10_SENSOR_ID) && (s_ctrl->io_master_info.master_type == CCI_MASTER) && (NULL != s_ctrl->io_master_info.cci_client)) {
             unsigned short i2c_addr_temp = s_ctrl->io_master_info.cci_client->sid;
             uint32_t supplier_id = 0;
             s_ctrl->io_master_info.cci_client->sid = 0xB0 >> 1;
             camera_io_dev_read(&(s_ctrl->io_master_info),0x000B,&supplier_id,2,1);
             CAM_INFO(CAM_SENSOR, "OV13B10 supplier_id = 0x%x",supplier_id);
             s_ctrl->io_master_info.cci_client->sid = i2c_addr_temp;
             if ((OFILM_SUPPLIER_ID == supplier_id) && (OV13B10_OFILM_I2C_ADDR != slave_info->sensor_slave_addr)) {
                  CAM_ERR(CAM_SENSOR, "sensor_slave_addr = 0x%d,supplier_id = 0x%x not matched",slave_info->sensor_slave_addr,supplier_id);
                  rc = -ENODEV;
               }
             if ((TXD_SUPPLIER_ID == supplier_id) && (OV13B10_TXD_I2C_ADDR != slave_info->sensor_slave_addr)) {
                  CAM_ERR(CAM_SENSOR, "sensor_slave_addr = 0x%x,supplier_id = 0x%x not matched",slave_info->sensor_slave_addr,supplier_id);
                  rc = -ENODEV;
              }
        }
    #endif
	if ((0x0d42 == slave_info->sensor_id) && (0x6C == slave_info->sensor_slave_addr)){
		hardwareinfo_set_prop(HARDWARE_BACK_CAM,"One_Back_oflim_Ov13b10");
		hardwareinfo_set_prop(HARDWARE_BACK_CAM_MOUDULE_ID,"Oflim");
	} else if ((0x0d42 == slave_info->sensor_id) && (0x20 == slave_info->sensor_slave_addr)){
		hardwareinfo_set_prop(HARDWARE_BACK_CAM,"Two_Back_txd_Ov13b10");
		hardwareinfo_set_prop(HARDWARE_BACK_CAM_MOUDULE_ID,"tongxingda");
	} else if ((0x08a3 == slave_info->sensor_id) && (0x62 == slave_info->sensor_slave_addr) && (slave_info->camera_id == 0)){
		hardwareinfo_set_prop(HARDWARE_BACK_CAM,"Two_Back_qtech_Gc08a3");
		hardwareinfo_set_prop(HARDWARE_BACK_CAM_MOUDULE_ID,"qtech");
	} else if ((0x08a3 == slave_info->sensor_id) && (0x62 == slave_info->sensor_slave_addr) && (slave_info->camera_id == 2)){
		hardwareinfo_set_prop(HARDWARE_FRONT_CAM,"Two_Front_lce_Gc08a3");
		hardwareinfo_set_prop(HARDWARE_FRONT_CAM_MOUDULE_ID,"lcetron");
	} else if ((0x487B == slave_info->sensor_id) && (0x20 == slave_info->sensor_slave_addr) && (slave_info->camera_id == 2)){
		hardwareinfo_set_prop(HARDWARE_FRONT_CAM,"One_Front_qtech_S5K4H7");
		hardwareinfo_set_prop(HARDWARE_FRONT_CAM_MOUDULE_ID,"qtech");
	} else if ((0x487B == slave_info->sensor_id) && (0x20 == slave_info->sensor_slave_addr) && (slave_info->camera_id == 0)){
		hardwareinfo_set_prop(HARDWARE_BACK_CAM,"One_Back_kks_S5K4H7");
		hardwareinfo_set_prop(HARDWARE_BACK_CAM_MOUDULE_ID,"Kingcom");
