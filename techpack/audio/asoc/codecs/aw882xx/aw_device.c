...
int aw_dev_init_cali_f0(struct aw_device *aw_dev)
{
        int ret = 0;

        if (aw_dev->cali_desc.mode) {
                if (aw_dev->cali_desc.cali_f0 == AW_ERRO_CALI_VALUE) {
                        ret = aw_cali_read_f0_from_nvram(&aw_dev->cali_desc.cali_f0, aw_dev->channel);
                        if (ret) {
                                aw_dev_info(aw_dev->dev, "read nvram cali failed, use default F0");
                                aw_dev->cali_desc.cali_f0 = AW_ERRO_CALI_VALUE;
                        }
                }
        } else {
                aw_dev_info(aw_dev->dev, "no cali, needn't init cali F0");
        }
        return ret;
}
