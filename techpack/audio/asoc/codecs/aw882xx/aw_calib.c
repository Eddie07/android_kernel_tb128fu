...
#define AWINIC_CALI_F0_FILE  "/mnt/vendor/persist/audio/aw_cali_f0.bin"
...
static int aw_cali_write_cali_f0_to_file(int32_t cali_f0, int channel)
{
        ...
        if (IS_ERR(fp)) {
                pr_err("%s:channel:%d open %s failed!\n",
                __func__, channel, AWINIC_CALI_F0_FILE);
                return -EINVAL;
        }
        ...
        pr_info("%s: channel:%d buf:%s cali_f0:%d\n",
                __func__, channel, buf, cali_f0);
...
static int aw_cali_get_read_cali_f0(int32_t *cali_f0, int channel)
{
	...
	int f_size;
        ...
        if (sscanf(buf, "%d", &int_cali_f0) == 1)
                *cali_f0 = int_cali_f0;
        else
                *cali_f0 = AW_ERRO_CALI_VALUE;

        pr_info("%s: channel:%d buf:%s int_cali_re: %d\n",
                __func__, channel, buf, int_cali_f0);
...
int aw_cali_read_f0_from_nvram(int32_t *cali_f0, int32_t channel)
{
...
static int aw_cali_svc_dev_get_f0(struct aw_device *aw_dev)
{
	...
	ret = aw_cali_write_f0_to_nvram(aw_dev->cali_desc.cali_f0, aw_dev->channel);
	if (ret)
