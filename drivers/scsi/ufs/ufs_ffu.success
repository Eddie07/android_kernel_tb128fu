...
static int ufs_scsi_write_partition(struct scsi_device *sdev, void *buf, uint32_t lba, uint32_t blocks)
{
    ...
    if (!ret && !scsi_device_online(sdev)) {
        ...
        pr_err("Murphy.liu note:get device fail\n");
    }
    ...
    if (ret) {
        pr_err("Murphy.liu note:[ufs_ffu] write error %d\n", ret);
    }
...
static int ufs_write(struct scsi_device *sdev, void *buf, uint32_t lba, uint32_t buf_size)
{
    ...
    while (block_count > 0) {
        ...
        if (rc != 0) {
            pr_err("Murphy.liu note:[ufs_ffu] write partition error %d\n", rc);
            return 1;
        }
    }
    return 0;
...
static int ufs_scsi_read_partition(struct scsi_device *sdev, void *buf, uint32_t lba, uint32_t blocks)
{
    ...
    if (ret) {
        pr_err("Murphy.liu note:[ufs_ffu] read error %d\n", ret);
    }
...
static int ufs_read(struct scsi_device *sdev, void *buf, uint32_t lba, uint32_t buf_size)
{
    ...
    while (block_count > 0) {
        ...
        if (rc != 0) {
            pr_err("Murphy.liu note:[ufs_ffu] read partition error %d\n", rc);
            return 1;
        }
    }
    return 0;
...
static struct fw_update_checklist need_update_k[] = {
    ...
    {"SAMSUNG\0", "KM5P9001DM-B424\0", "0100","0300",
    (uint64_t)Samsung_km5pB514, sizeof(Samsung_km5pB514)},
};
...
static int get_fw_update_checklist(struct scsi_device *sdev, struct fw_update_checklist **need_update, struct ffu_inquiry *stdinq)
{
    ...
    if (rc) {
        pr_err("Murphy.liu note:[ufs_ffu] get partion info error %d\n", rc);
    ...
    if (strncmp(part_head.ffu_flag, FFU_FLAG, sizeof(FFU_FLAG))) {
        pr_err("Murphy.liu note:[ufs_ffu] ufs_flag is not FFU %s\n", part_head.ffu_flag);
        *need_update = need_update_k;
        goto out;
    }

    pr_err("Murphy.liu note:ufs_ffu] ffu_count is %d\n", part_head.ffu_count);
    ...
    *need_update = kmalloc(list_count*sizeof(struct fw_update_checklist), GFP_KERNEL);
    ...
    for (i = 0; i < part_head.ffu_count; i++) {
        ...
        pr_err("Murphy.liu note:[ufs_ffu] get info from partition crc 0x%lx addr 0x%llx size 0x%x\n", *crc, fw_info.fw_addr, fw_info.fw_size);

        if (!fw_info.fw_addr || fw_info.fw_addr % PART_BLOCK_SIZE) {
            pr_err("Murphy.liu note:[ufs_ffu] ffu bin is not 4K align 0x%llx\n", fw_info.fw_addr);
            continue;
        }
        ...
        if (rc) {
            pr_err("Murphy.liu note:[ufs_ffu] get fw bin info error %d\n", rc);
            continue;
        }

        if (*crc != crc32(fw, fw_info.fw_size)) {
            pr_err("Murphy.liu note:[ufs_ffu] crc error 0x%x diff 0x%x\n", *crc ,crc32(fw, fw_info.fw_size));
            kfree(fw);
            continue;
        }
    ...
    pr_err("Murphy.liu note:[ufs_ffu] partion start %d size %d\n", part_info.part_start, part_info.part_size);
out:
    kfree(buf);
    return ret;
}
...
static int32 ufs_fw_update_write(struct scsi_device *sdev, int32_t size, uint8_t *buf)
{
    ...
    if (!buf_ptr) {
        pr_err("Murphy.liu note:ffu kzalloc fail.\n");
        rc = -1;
        return rc;
    }

    memcpy(buf_ptr, buf, size);
    pr_err("Murphy.liu note:fw addr: %p, %p. total fw size: %d\n", buf_ptr, buf, size);

    for (offset = 0; offset + WB_SIZE <= size; offset += WB_SIZE) {
        pr_err("Murphy.liu note:ffu offset:%d, size%d\n", offset, WB_SIZE);
...
static int check_ffu_parition_fw_error(struct scsi_device *sdev, int index)
{
    ...
    if (index >= update_k_size) {
        ...
        if (rc) {
            pr_err("Murphy.liu note:[ufs_ffu] write partion info error %d\n", rc);
        }
    }
...
int ufs_ffu(struct scsi_device *sdev)
{
    ...
    pr_err("Murphy.liu note:come in ufs_ffu\n");
    if (off_charge_flag) {
        pr_err("Murphy.liu note:ufs_ffu enter off charger\n");
        return 0;
    }

    pr_err("Murphy.liu note:start to FFU 0x%x\n", ffu_get_flag());

    if (one_time_entry) {
        pr_err("Murphy.liu note:[ufs_ffu] one time entry is true\n");
        return 0;
    } else {
        one_time_entry = true;
    }
	...
	pr_err("Murphy.liu note:sdev->host->hostt->proc_name is NULL\n");
        ret = -1;
        return ret;
    }
    if (strncmp(sdev->host->hostt->proc_name, UFSHCD, strlen(UFSHCD))) {
        /*check if the device is ufs. If not, just return directly.*/
	pr_err("Murphy.liu note:sdev->host->hostt->proc_name is not UFSHCD\n");
        ret = -1;
        return ret;
    }
    ...
    pr_err("Murphy.liu note:[ufs_ffu] check ffu_arry, vendor:%s, product:%s, fw_rev:%s\n", stdinq.vendor_id, stdinq.product_id, stdinq.product_rev);
    ...
    if (qpnp_pon_uvlo_get) {
        if (ffu_get_flag() == 1) {
            pr_err("Murphy.liu note:fail flag was set in last boot. skip FFU.\n");
            ret = ffu_set_flag(0);
            ret = -1;
            return ret;
        }
    } else {
        pr_err("Murphy.liu note:fail flag is not supported\n");
    }

    pr_err("Murphy.liu note:in ufs_ffu,get_fw_update_checklist\n");
	...
	pr_err("Murphy.liu note:step 1. match vendor id && product id \n");
        if (strncmp((char *)stdinq.vendor_id, (char *)need_update[i].vendor_id, strlen(need_update[i].vendor_id)) == 0
            ...
            pr_err("Murphy.liu note:match vendor:%s, product id:%s hw_sectors %d\n", stdinq.vendor_id, stdinq.product_id,
                queue_max_hw_sectors(sdev->request_queue));

            /*step 2.  match product revision from */
	    pr_err("Murphy.liu note:step 2.  match product revision from \n");
            if (strncmp((char *)stdinq.product_rev, (char *)need_update[i].product_rev_from, strlen(need_update[i].product_rev_from)) == 0) {
                ...
                pr_err("Murphy.liu note:match product revision from:%s\n", stdinq.product_rev);
                pr_err("Murphy.liu note:start write buffer\n");
                pr_err("Murphy.liu note:fw_size = %d, address = %p\n", need_update[i].fw_size, need_update[i].fw_addr);
                ret = ufs_fw_update_write(sdev, need_update[i].fw_size, (uint8_t *)need_update[i].fw_addr);
                if (ret == 0) {
                    pr_err("Murphy.liu note:write buffer success\n");
                    pr_err("Murphy.liu note:clear fail flag, check revision after reboot\n");
                    if (qpnp_pon_uvlo_set) {
                        ffu_set_flag(0);
                    }
                    ...
                    if (qpnp_pon_uvlo_get) {
                        pr_err("Murphy.liu note:ufs ffu failed, don't clear fail flag, reboot directly\n");
                        check_ffu_parition_fw_error(sdev, i);
                        msleep(20);
                        machine_restart("ufs ffu reboot");
                    }
                    pr_err("Murphy.liu note:ufs ffu failed, do not reboot to avoid endless reboot\n");
                }
                break;
                ...
                pr_err("Murphy.liu note:ufs ffu firmware is already the newest  now\n");
                continue;
            } else {
                pr_err("Murphy.liu note:ufs_ffu do not match the rev\n");
            }
        } else {
            pr_err("Murphy.liu note:ufs ffu do not match %d:%s %s\n", i, need_update[i].vendor_id, need_update[i].product_id);
        }
    }
